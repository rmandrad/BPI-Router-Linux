			    ...
			    bool cac_ongoing)
{
	...
	/* Keep the timestamp consistent across all subchannels. */
			      ...
			      const struct cfg80211_chan_def *chandef)
{
	...
	for_each_subchan(chandef, freq, cf) {
		...
		if (reg_get_dfs_region(wiphy) == NL80211_DFS_ETSI &&
			...
			if (cf == 1 && t1 < IEEE80211_DFS_WEATHER_MIN_CAC_TIME_MS)
				t1 = IEEE80211_DFS_WEATHER_MIN_CAC_TIME_MS;
			if (cf == 2 && t2 < IEEE80211_DFS_WEATHER_MIN_CAC_TIME_MS)
				t2 = IEEE80211_DFS_WEATHER_MIN_CAC_TIME_MS;
		}

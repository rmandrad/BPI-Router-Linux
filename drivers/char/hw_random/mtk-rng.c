...
static bool mtk_rng_hw_accessible(struct mtk_rng *priv)
{
	...
	/*
	 * If TF-A blocks direct access, the register reads back as 0. Leave the
	 * clock enabled because TF-A's SMC handler needs it.
	 */
	return false;
}
...
static int mtk_rng_probe(struct platform_device *pdev)
{
	...
	priv->flags = (unsigned long)of_device_get_match_data(&pdev->dev);
